...
def render_trend_dashboard() -> None:
    ...
    if trend_df is None or trend_df.empty:
        st.info("No trend data yet. Generate dashboard more than once to see trend.")
    ...
    st.caption("Trend is based on each SQLite run_id generated from uploaded Excel files.")

    key_action_df = build_key_action_trends(action_trend_df)

    if key_action_df is not None and not key_action_df.empty:
